...
description: 生成 Ian 风格的正文配图。用户提供链接或原文时，先读文确认理解并用问答锁定 IP/风格/图上文字语言/张数/模式，再按回答规划或生图；也支持小黑/便签/句号、纯白手绘、中/英/双语标注、shot list、去标题改图等。默认小黑 + 纯白手绘 + 中文标注。
---
...
- IP：`references/ips/catalog.md`（`xiaohei` / `note` / `dot`）
- 风格：`references/styles/catalog.md`（目前 `white-handdrawn`）
