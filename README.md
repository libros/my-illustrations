# Joel Article Illustrations

> 把中文文章里的判断、流程、状态和隐喻，变成一张张白底、手绘、怪诞但清爽的正文配图。
>
> 16:9 横版 | 可选 IP（默认小黑） | 纯白手绘 | 图上文字默认可中文 | Codex Skill：`$joel-article-illustrations`

---

## 这个仓库是什么

Joel Article Illustrations 是一个 Codex Skill（安装名：`joel-article-illustrations`），用来指导 AI Agent 为中文文章、帖子、博客、Notion 文档和方法论内容生成正文配图。

它不是通用插画 prompt，也不是 PPT 信息图模板。它的核心目标是：先理解文章里的认知锚点，再把其中一个判断、流程、结构、状态或隐喻，变成一张有记忆点的 16:9 手绘解释图。

默认视觉 IP 是“小黑”（也可选便签、句号等变体）：黑色实心、白点眼、细腿、空表情，认真做荒诞但成立的事。角色不是吉祥物，必须参与核心动作。

使用时默认会先读文、确认理解，并用问答锁定 IP / 风格 / 图上文字语言 / 张数，再规划或生图。
...
在仓库根目录，把 **skill 子目录** 拷到 Codex skills（包名已与旧版小黑 skill 区分）：

```bash
mkdir -p "${CODEX_HOME:-$HOME/.codex}/skills"
# 若曾安装旧名，建议先删掉以免冲突：
# rm -rf "${CODEX_HOME:-$HOME/.codex}/skills/ian-xiaohei-illustrations"
cp -R ./joel-article-illustrations "${CODEX_HOME:-$HOME/.codex}/skills/"
```

安装后，在 Codex 里使用（新调用名，不要用 `$ian-xiaohei-illustrations`）：

```text
Use $joel-article-illustrations
下面是文章链接或原文。请先读文确认，再问答让我选 IP、风格、图上文字语言、张数和模式。
```
...
```text
Use $joel-article-illustrations 先不要生图。
...
```text
Use $joel-article-illustrations 把下面这篇文章生成 4 张小黑怪诞正文配图。
...
```text
Use $joel-article-illustrations 为“信任不是喊出来的，而是一块证据一块证据铺过去”生成一张正文配图。
画面要怪诞但清爽，小黑必须承担核心动作。
```
...
```text
Use $joel-article-illustrations 帮我编辑这张图，去掉左上角的“流程图”标题，其他内容保持不变。
```
...
│   │   └── ...
│   └── prompts.md
└── joel-article-illustrations/     ← 安装这个目录到 Codex skills
    ├── SKILL.md
    ├── agents/openai.yaml
    ├── assets/examples/
    └── references/
...
调用名：`$joel-article-illustrations`（小黑只是默认 IP，不是 skill 名）。
